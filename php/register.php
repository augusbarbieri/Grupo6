...
if ($nombre === '' || $apellido === '' || $email === '' || $password === '' || $password_r === '') {
    echo 'Faltan campos obligatorios. <a href="../paginas/registro.html">Volver</a>';
    exit();
}

if ($password !== $password_r) {
    echo 'Las contraseñas no coinciden. <a href="../paginas/registro.html">Volver</a>';
    exit();
}

// Preparar ruta de archivos (usar ruta absoluta en servidor)
$upload_dir = __DIR__ . '/../archivos/perfiles/';
if (!is_dir($upload_dir)) {
    mkdir($upload_dir, 0755, true);
}

$path_imagen_bd = "archivos/perfiles/default.png"; // por defecto
if (isset($_FILES["img"]) && $_FILES["img"]["error"] === UPLOAD_ERR_OK) {
    $nombre_original = basename($_FILES["img"]["name"]);
    $extension = strtolower(pathinfo($nombre_original, PATHINFO_EXTENSION));
    $allowed = ['jpg', 'jpeg', 'png', 'webp', 'gif'];
    if (!in_array($extension, $allowed)) {
        echo 'Tipo de archivo no permitido. <a href="../paginas/registro.html">Volver</a>';
        exit();
    }
    // Limitar tamaño ejemplo 5MB
    if ($_FILES["img"]["size"] > 5 * 1024 * 1024) {
        echo 'Imagen demasiado grande. <a href="../paginas/registro.html">Volver</a>';
        exit();
    }
    ...
    if (move_uploaded_file($_FILES["img"]["tmp_name"], $path_completo)) {
        $path_imagen_bd = 'archivos/perfiles/' . $nombre_unico; // ruta relativa para guardar en BD
    } else {
...
if ($filasAfectadas > 0) {
    ...
    echo 'Hubo un error al crear la cuenta. <a href="../paginas/registro.html">Volver</a>';
}
